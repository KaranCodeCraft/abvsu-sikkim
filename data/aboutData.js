// data/flyout/aboutData.js

const AboutData = [
    {
      title: "About",
      items: [
        {
          name: 'About ABVSU',
          link: '/aboutABVSU'
        },
        {
          name: 'About Sikkim',
          link: '/aboutsikkim'
        },
        {
          name: 'Institutional Development Plan',
          link: '/institutionaldevelopmentplan'
        },
        // {
        //   name: 'Accreditation',
        //   link: '/accreditation'
        // },
        {
          name: 'Our Inspiration',
          link: '/ourinspiration'
        },
        {
          name: 'Why ABVSU',
          link: '/whyabvsu'
        },
        {
          name: 'Vision and Mission',
          link: '/visionandmission'
        },
      ],
    },
    {
      title: "Leadership",
      items: [
        {
          name: `Vice Chancellor's Message`,
          link: '/vicechancellormessage'
        },
        {
          name: 'Registrar Message',
          link: '/registrarmessage'
        },
      ],
    },
    {
      title: "University Committe",
      items: [
        {
          name: 'Anti-Ragging Commitee',
          link: '/#'
          // link: '/antiraggingcomitee'
        },
        {
          name: 'Discipline Committee',
          link: '/disciplinecommittee'
        },
        {
          name: 'Internal Complaint Committee',
          link: '/internalcomplaintcommittee'
        },
        {
          name: 'Event Organisation Commitee',
          link: '/eventorganisationcommittee'
        },
        {
          name: 'Student Welfare Committee',
          link: '/studentwelfarecommittee'
        },
        {
          name: 'Seminar Committee',
          link: '/seminarcommittee'
        },
        {
          name: 'Sports Club Committee',
          link: '/sportsclubcommittee'
        },
        {
          name: 'Women Sexual Harrasment Committee',
          link: '/womensexualharassmentcommittee'
        },
        {
          name: 'Internal Quality Assurance Cell',
          link: '/internalqualityassurancecell'
        },
      ],
    },
    {
      title: "Authorities @ ABVSU",
      items: [
        {
          name: 'Governing Body',
          link: '/governing-body'
        },
        // {
        //   name: 'Executive Council',
        //   link: '/research'
        // },
        {
          name: 'Academic Council',
          link: '/academic-council'
        },
        {
          name: 'Board of Research Studies',
          link: '/board-of-research-studies'
        },
        {
          name: 'The Finance Committee',
          link: '/finance-committee'
        },
        {
          name: 'Board of Studies',
          link: '/board-of-studies'
        },
      ],
    },
    
  ];
  
  export default AboutData